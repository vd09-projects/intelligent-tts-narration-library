...
func (a flagSet) validate() error {
	...
	if a.Listen {
		if info, rerr := pipeline.ResolveVoice(a.effectiveVoice()); rerr == nil && info.RequiresWorker {
			...
			return fmt.Errorf("--listen cannot be combined with --voice=%s (this voice renders at %d kHz, worker-backed; listen plays through a fixed 24 kHz context — pending dynamic oto rate, #154)", a.effectiveVoice(), info.Format.SampleRate/1000)
		}
	}
	return nil
}
