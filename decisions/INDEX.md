...
```yaml
decisions:
  - id: 2026-07-28-gso-164-machine-go-but-full-close-gated-on-by-ear
    title: "GSO #164 go/no-go — machine-evidence GO on the hard RAM ceiling, but the full close is gated on human by-ear"
    date: 2026-07-28
    status: accepted
    category: tradeoff
    ...
    path: tradeoff/2026-07-28-gso-164-machine-go-but-full-close-gated-on-by-ear.md
    summary: "#164 (GSO P4) go/no-go gate — an honest-ceiling outcome, not a clean pass, recorded per AC6 regardless of outcome and mirroring the #147 honesty-over-optimism precedent. Machine go/no-go = GO on the HARD input (peak RSS): official make gso-perf-baseline (M1 Pro, docs/samples/sample.md, 15 blocks, 2026-07-28) measured peak RSS 2.13 GB vs the <=8 GB ceiling with comfortable headroom (cross-checked getrusage ru_maxrss 2.12 GB; Apple-Silicon unified-memory/MPS caveat noted — RSS can under-count GPU-side allocations, near-ceiling reads reported conservatively, but 2.13 GB is not near-ceiling so the caveat doesn't flip the verdict). Latency is INFORMATIONAL never a gate (warm-load-proof correctness-oracle standing order): cold load 22.67s (<=30s OK); warm-per-block mean 24.49s is OVER the ~20s advisory ceiling driven by long verbatim-prose outliers (block 3 = 161.12s, block 1 = 59.80s; median ~3s; spread 2.41-161.12s) — documented, not a blocker. G2P coverage (AC3, mechanized) half A (Segment.Text correctness from plan.json) PASS all 5 structured classes (24,700 -> 'twenty-four thousand seven hundred'; 1,000,000 left as digits at the 6-digit ceiling; code gist; list item-by-item; table header+rows at L3; heading 'Section:'-prefixed); half B (g2p_en phoneme vs ARPAbet) documents TWO ceilings — CEILING 1 hyphenated spelled-out cardinals mis-phonemized ('twenty-four' -> ...F UH1 R vs correct F AO1 R; 'forty-two'; code-gist '4-line'; the hyphen is the trigger, bare 'four' correct); CEILING 2 technical tokens/identifiers mangled ('L1' -> 'lone'; '32 kHz'; 'GPT-SoVITS'; 'cool-jahns-gso' garbled) — the NEW risk RVC never had (RVC inherited Kokoro pronunciation for free; GSO's own g2p_en does not). By-ear legs are human-owned, never a machine verdict: AC1 CLI STAGED pending ears (audio.wav produced, objective signals all pass — 32 kHz mono s16le, manifest voice==cool-jahns-gso, one BlockTiming per block, non-silent RMS 0.104/peak 0.883); AC2 MCP BLOCKED not staged (make mcp-voice-sanity MCP_VOICE=cool-jahns-gso failed at 49.66s — TestSpeakManualSmoke hardcodes docs/samples/sample.md whose long-prose blocks exceed the 30s per-block renderer timeout, context deadline exceeded -> follow-ups: timeout scaling, smoke-source override). All acoustic judgments (Jeremy Jahns likeness, whether the g2p ceilings degrade audio, gist intelligibility) are PENDING HUMAN EARS and explicitly NOT inputs to the machine go/no-go. DECISION (mirrors #147): ship the faithful machine-checkable work behind the human by-ear gate — GO on the hard RAM ceiling with headroom; latency over-run and g2p_en ceilings documented not fabricated as blockers; the FULL close depends on the human's by-ear sign-off of the CLI artifact; character/quality fixes and the MCP leg spin to follow-ups. Honesty over optimism — no by-ear pass recorded that the agent cannot hear."
  ...
  - id: 2026-07-28-gso-render-peer-renderer-not-rvc-decorator
    ...
    path: architecture/2026-07-28-gso-render-peer-renderer-not-rvc-decorator.md
    summary: "#162 builds render/gptsovits, the Go GPT-SoVITS engine, and had to reconcile two competing structural precedents: render/sherpa (Kokoro) is a SOURCE engine that owns the block loop, reads plan text via spokenTextFor, and emits Timeline/manifest from scratch, wrapping nothing; render/rvc is a DECORATOR that wraps an inner render.Renderer and repaints its already-rendered WAVs (audio->audio), never seeing plan text. The standing RVC precedent said 'wrap the worker as a decorator over the base engine.' DECISION: render/gptsovits implements render.Renderer DIRECTLY as a PEER modeled on sherpa, and is deliberately NOT an rvc-style decorator — build_renderer.go's EngineGSO arm returns a bare gptsovits.New(...) with no inner renderer to wrap (var _ render.Renderer = (*Engine)(nil)). Decisive reason: GPT-SoVITS is TEXT->AUDIO (like Kokoro), not AUDIO->AUDIO (like RVC), so there is no base-engine audio to repaint — wrapping Kokoro would synthesize every block TWICE and force the decorator to reach past its inner renderer for the plan text it hides. This scopes the RVC 'wrap as decorator' standing order to audio->audio conversion only, not to peer TTS engines. The synthesis that landed is a SHERPA-SHAPED peer renderer (block loop, spokenTextFor with refused blocks voicing Refusal.Message, empty-text zero-dur skip still present in Timeline.Blocks, cursorMs accumulation, from-scratch Timeline/manifest, voice-neutral via RenderOptions.Voice keeping the roster the single validator) driven by an RVC-SHAPED warm subprocess worker (one warm process per document per AC2 so the ~17-22s cold load is paid once not per block, goroutine lockstep exchange, exit-code taxonomy, clean stdin-close reap; RenderBlock = single cold-load exchange mirroring RVC's). The one place sherpa's shape was insufficient — sherpa spawns a subprocess PER BLOCK, which would repay the cold load every block — is grafted with the warm worker, not hidden. Three GSO wire deltas from RVC transport (per the gso-worker-wire-contract-rvc-shaped-not-verbatim decision): no <out> token (worker mints a content-addressed path, echoes it), OK payload is literal line[3:] never shlex-split, and D4 content-addressed buffer-before-next-request (align each OK's WAV before the next exchange since identical blocks mint the same file). REJECTED: an rvc-style decorator over base Kokoro (structurally wrong for text->audio; double synthesis). Consequences: only the two S1 seams (voices.go + build_renderer.go) know the concrete engine, planner//plan/ byte-untouched; writeManifest is now a third // DUP: copy (extract to render/internal only on a fourth producer); reviewers diff the renderer body vs render/sherpa/sherpa.go and the worker vs render/rvc/worker.go; a future engineer must not unify GSO into the RVC decorator arm — the text->audio vs audio->audio split is the load-bearing reason they differ. Departs from the torch-free-onnx-rvc-ephemeral-worker decorator precedent; consumes the frozen #161 gso-worker-wire-contract; wired via the pipeline-hosts-buildrenderer-factory."
  ...
  - id: 2026-07-27-gso-worker-wire-contract-rvc-shaped-not-verbatim
    ...
    path: architecture/2026-07-27-gso-worker-wire-contract-rvc-shaped-not-verbatim.md
    summary: "#161 GSO worker (scripts/gptsovits_worker.py, torch subprocess over TTS_infer_pack) speaks a wire contract that REUSES the RVC v1 *shape* (decision 2026-07-22-rvc-worker-wire-contract-err-taxonomy-exit-codes: stdin line loop, positional append-only tokens, exactly one response line per request, CLOSED ERR taxonomy {bad-args|bad-voice|read-failed|infer-failed|write-failed}, 78-startup/70-runtime FATAL exit split, native abort → process dies with no line → #162 fatal hard-stop) but DELIBERATELY DIVERGES in three GSO-specific ways: (1) the worker MINTS its own output path — AC2's line has no <out> token (RVC took a caller-supplied <out>), so the worker resolves GSO_OUT_DIR-or-mkdtemp, derives a content-addressed gso-<voice_id>-<sha256(canonical tuple)[:16]>.wav, idempotent atomic overwrite (temp-write+os.replace), 64-bit digest so collisions 'won't in practice at this scale' not 'cannot', worker NEVER deletes/holds-no-TTL, #162 owns full cleanup lifecycle, and the worker is a de-facto content-addressed cache (identical requests → ONE physical file, so #162 must NOT assume one file per .run()); (2) consumes .ckpt/.pth DIRECTLY, never ONNX (inverse of RVC's torch-free ONNX; torch asserted PRESENT at startup, reached only across the subprocess boundary); (3) emits a THIRD system sample rate 32 kHz (Kokoro 24 kHz, RVC its own), and since CLAUDE.md forbids resampling the #162/#163 roster metadata MUST carry per-engine sample rate. Fourth practical divergence: GSO's <text>/<prompt_text> are free-text with spaces so shlex-quoting is LOAD-BEARING (RVC tokens were space-free); Go has no stdlib shlex quoter so #162 must vendor one and validate against the shared golden fixture scripts/testdata/gso-shlex-golden.json. This is the FROZEN v1 contract #162 binds to; evolution constrained to trailing-optional-token-with-proven-default, ERR set append-only, no reorder/repurpose. REJECTED verbatim RVC reuse (factually impossible — round-1 plan-review blocker) and a from-scratch unrelated contract (discards proven framing). Sibling to the GSO wire-vs-packaged ref/prompt precedence decision; cross-references the RVC wire-contract and torch-free-ONNX-worker precedents."
  - id: 2026-07-27-gso-wire-vs-packaged-ref-prompt-precedence-drift
    title: "GSO ref_audio/prompt_text has two homes (wire + packaged) — wire is authoritative, drift maps onto the closed taxonomy, single-source redesign deferred to an AC edit"
    date: 2026-07-27
    status: accepted
    category: architecture
    ...
    path: architecture/2026-07-27-gso-wire-vs-packaged-ref-prompt-precedence-drift.md
    summary: "#161's two LOCKED acceptance criteria create two homes for the same reference data: AC1 packages a per-voice ref_audio.wav + ref_transcript.txt (resolvable from <voice_id>), AC2 carries <ref_audio_path> + <prompt_text> on the wire. Plan-review round 1 flagged this as two sources of truth with no drift ERR. Because both ACs are locked ticket text, the plan KEEPS BOTH homes and defines their relationship rather than dropping either. DECISION: (precedence) the WIRE tokens are AUTHORITATIVE for what is fed to .run(); the packaged artifacts are the source-of-record / DR backup and the canonical values #162 SHOULD emit. (drift, mapped onto the CLOSED taxonomy — no new category invented) <ref_audio_path> resolving into a DIFFERENT voice's packaged dir than <voice_id> → bad-voice; <ref_audio_path>/<prompt_text> inconsistent with THIS voice's packaged artifacts (path outside the voice dir, or prompt_text mismatches packaged ref_transcript.txt on a best-effort compare) → bad-args. The best-effort prompt_text compare is a COARSE guardrail against gross drift (wrong-voice/out-of-dir), NOT transcript enforcement — the wire prompt_text wins and is voiced as given, so subtle transcript edits still pass; byte-for-byte transcript verification is explicitly not mandated in phase one. The cleaner single-source-of-truth redesign (drop the wire tokens, resolve everything from <voice_id>, which makes wire==packaged by construction) is EXPLICITLY DEFERRED to a ticket-owner AC1/AC2 edit + wire re-freeze — recorded as a recommendation, NOT done silently. REJECTED: Option A single-source now (needs an AC edit, out of #161 scope); Option B two homes with no precedence (the round-1 blocker). Sibling to the GSO RVC-shaped-not-verbatim wire-contract decision; drift is mapped onto the same closed taxonomy the RVC wire-contract decision established."
  - id: 2026-07-27-gso-warmproof-warm-reuse-correctness-oracle
    title: "GSO warm-load proof is a correctness oracle (A,B,A determinism + warm-vs-cold), buffered before overwrite, guarded by a negative dry-check — not a latency check"
    date: 2026-07-27
    status: accepted
    category: convention
    ...
    path: convention/2026-07-27-gso-warmproof-warm-reuse-correctness-oracle.md
    summary: "#161's single novel risk (AC5): proving ONE warm TTS_infer_pack pipeline serves N sequential .run() calls CORRECTLY (not just faster), which the runbook never exercised (one .run() per fresh process). The first plan gated on a LATENCY check; plan-review round 1 rejected it as near-tautological (request-1 always pays one-time model load) and 'outputs differ per input' as necessary-not-sufficient. The replacement correctness oracles then COLLIDED with the worker's content-addressed idempotent-overwrite path (A1/A2 mint the same file, A2 clobbers A1) → a naive re-read degenerates to assert x==x, a FALSE GREEN on the exact warm-state corruption AC5 exists to catch (round-2 blocker). DECISION (standing test methodology): make gso-warmproof is a CORRECTNESS oracle, not a latency check — (primary) A,B,A repeat-input determinism assert bytes(A1)≈bytes(A2) within an RMS/spectral tolerance (seeded, not byte-equal — MPS not bit-exact); (secondary) warm-vs-cold equivalence comparing the genuinely-warm Nth call (pcm_a2) vs a fresh cold single-shot, warm and cold using DISTINCT GSO_OUT_DIRs so identical input can't clobber across processes; (load-bearing fix) READ-AND-BUFFER each OK <out>'s WAV bytes the instant its line arrives, BEFORE the next request — compare buffered bytes, never re-read a content-addressed path after a later identical request (relies on OK emitted only after the atomic write → correct happens-before that closes the vacuous self-compare window); the worker's content-addressing/overwrite is deliberately kept intact (correct for dedup), fix is HARNESS-ONLY; (self-test) a NEGATIVE DRY-CHECK runs the oracle against a state-leaking fake stub and asserts it goes RED — a green result is only trusted once the oracle proves it CAN fail, and it doubles as the regression tripwire against any future re-introduction of the self-compare (torch-free, provable in-session). Latency is informational-only, never a gate. Honesty: negative dry-check + contract/parse/ERR layer provable in-session; AC5 actual green + AC1 ~950MB fetch are runtime-gated ('pending machine run', never faked). Builder+reviewer independently re-ran the negative dry-check green (RED at 2.01e-03 vs tol 1e-03, GREEN at 2.14e-04, RED gross, RED stale-cache). REJECTED: latency gate (tautological), re-read-after-run oracle (false-greens), nonce-the-paths (diverges from frozen content-addressing). Depends on the GSO RVC-shaped-not-verbatim wire-contract decision's content-addressed output path."
